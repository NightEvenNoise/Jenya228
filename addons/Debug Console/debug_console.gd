...
onready var android_controls = get_parent().get_node("ControlAndroid")
...
#func _input(event):
#	if self.visible == false && enable_console == true:
#		if event is InputEventKey && event.is_action_pressed("debug") && \
#		!event.is_echo():
#			self.grab_focus()
#			self.visible = true
#			output.visible = true
...
func commands(prefix: String, command: String = "", Option: String = ""):
	...
	match prefix: # Match the prefix, then check for the relevant command
		...
		"scene":
			mod_alias("scene", command)
		"vsync":
			mod_window("vsync")
		"android_controls":
			mod_window("android_controls")
...
func mod_window(command: String):
	match command:
		...
		"android_controls":
			android_controls.visible = not android_controls.visible
		"vsync":
...
func mod_alias(command, value = null):
	match command:
		...
		"scene":
			match value:
				value:
					get_tree().call_deferred("change_scene",value+".tscn")
		"fxaa":
			match value:
				"":
